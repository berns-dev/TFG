"""Pruebas offline ronda 3 — solo teoría (materiales distintos a R1 y R2)."""
...
CASOS = [
    {
        ...
        "teoria": "Resortes_2025.pdf",
        "ronda1": "Frenos_2025.pdf",
        "ronda2": "Cojinetes_2025.pdf",
    },
    {
        "asignatura": "Oleohidráulica y Neumática",
        "carpeta": "Oleohidraulica",
        "teoria": "OyN_C3_Pneumatics_part_I.pdf",
        "ronda1": "OyN_C1_Hyd_and_Pneu_technologies.pdf",
        "ronda2": "OyN_C2_Hydraulic_Components_part_I.pdf",
    },
    {
...
def _probar_teoria(caso: dict, base: Path) -> dict:
    ...
    try:
        ...
        out["cnt_headings_falsos_prosa"] = [
            ...
            if any(
                v in h.lower()
                for v in ("muestra", "figura", "tabla", "recuérd", "observe", "mm de", "espesor")
            )
        ][:6]
        ...
        out["cnt_temario_curso"] = sum(
            1 for h in headings if re.search(r"tema\s+\d", h.lower())
        )
...
def main() -> None:
    ...
    out_path = DATOS / "_resultados_ronda3.json"
    out_path.write_text(json.dumps(informe, ensure_ascii=False, indent=2), encoding="utf-8")
    print(out_path)
